...
var debugActions = require('debug')('nightmare:actions');
...
// Blur input after text or insert
const DEFAULT_BLUR_INPUT = true;
...
function Nightmare(options) {
  ...
  this._debug = [];
  if (options.debug)
    this._debug.push(options.debug);

  this._log = [];
  if (options.log)
    this._log.push(options.log)

console.log("test")

  options.blurInput = options.blurInput || DEFAULT_BLUR_INPUT;
  ...
  if(options.loadTimeout && options.gotoTimeout && options.loadTimeout < options.gotoTimeout){
    this.debug(debug,`WARNING:  load timeout of ${options.loadTimeout} is shorter than goto timeout of ${options.gotoTimeout}`);
  }
  ...
  this.debug(debug,'queuing process start');
  this.queue((done) => {

    if (options.inProcess && typeof default_electron_path == 'object')
    {
      // Create runner instance using settings in options.inProcess
      var NightmareInProcess = require('./nightmare-inprocess');
      this.proc = new NightmareInProcess(options.inProcess);
      delete options.inProcess;
    }
    else // original Nightmare behaviour of launching new Electron process
    {
      var electronArgs = {};
    ...
    this.child.on('nightmare:ipc:debug',function() {
      self.log.apply(self,Array.from(arguments).slice(1));
    });
    ...
    this.child.on('page', function(type) {
      self.log.apply(self, ['page-' + type].concat(sliced(arguments, 1)));
    });

    // propogate events through to debugging
    this.child.on('did-finish-load', function () { self.log('did-finish-load', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-fail-load', function () { self.log('did-fail-load', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-fail-provisional-load', function () { self.log('did-fail-provisional-load', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-frame-finish-load', function () { self.log('did-frame-finish-load', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-start-loading', function () { self.log('did-start-loading', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-stop-loading', function () { self.log('did-stop-loading', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-get-response-details', function () { self.log('did-get-response-details', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('did-get-redirect-request', function () { self.log('did-get-redirect-request', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('dom-ready', function () { self.log('dom-ready', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('page-favicon-updated', function () { self.log('page-favicon-updated', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('new-window', function () { self.log('new-window', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('will-navigate', function () { self.log('will-navigate', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('crashed', function () { self.log('crashed', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('plugin-crashed', function () { self.log('plugin-crashed', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('destroyed', function () { self.log('destroyed', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('media-started-playing', function () { self.log('media-started-playing', JSON.stringify(Array.prototype.slice.call(arguments))); });
    this.child.on('media-paused', function () { self.log('media-paused', JSON.stringify(Array.prototype.slice.call(arguments))); });
  ...
  Object.keys(Nightmare.childActions).forEach(function(key){
    self.debug(debug,'queueing child action addition for "%s"', key);
...
function handleExit(code, instance, cb){
  ...
  self.debug(debug,'electron child process exited with code ' + code + ': ' + help[code]);
...
function endInstance(instance, cb, forceKill) {
  ...
  if (instance.proc && instance.proc.connected) {
    ...
    self.debug(debug,'electron child process not started yet, skipping kill.');
    cb();
  }
}
 ...
 * Version
 */
Nightmare.version = require('../package.json').version;
 ...
 * Debug
 */
Nightmare.prototype.debug = function(debug,...args)
{
  if (debug)
    debug.apply(debug,args);
  this._debug.forEach((extraDebug) => {
    extraDebug.apply(extraDebug,args)
  })
}
 ...
 * Log
 */
Nightmare.prototype.log = function(...args)
{
  log.apply(log,args);
...
Nightmare.prototype.setOptions = function(name,value)
{
  let options = {};
...
Nightmare.prototype.goto = function(url, headers,initFn) {
  this.debug(debug,'queueing action "goto" for %s', url);
  var self = this;
  ...
  this.queue(function(fn) {
    self.child.call('goto', url, headers, initFn,this.options.gotoTimeout, fn);
  });
  return this;
};

Nightmare.prototype.hide = function(done) {  
  this.child.call('hide', done);
  return this;
};

Nightmare.prototype.show = function(done) {  
...
Nightmare.prototype.evaluate_now = function(js_fn, done) {
  ...
  if (this.options.executeJavaScriptSecure)
  {
    var args = Array.prototype.slice.call(arguments).slice(2);
    this.child.call('javascript',String(js_fn),args,done);
  }
  else {
...
function queued (name, fn) {
  return function action () {
    this.debug(debug,'queueing action "' + name + '"');
