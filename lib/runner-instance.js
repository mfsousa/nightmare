...
var windowIpc = require('electron').ipcMain;
...
/**
 * Detect if an EventEmitter event of DOM event and abstract changes to call fn() with appropriate args
 * @param emitter
 * @param event
 * @param fn
 */
function handleEvent(emitter,event,fn)
{
...
function RunnerInstance(parent,inProcessOptions)
{
    ...
    if (inProcessOptions.window) {
      ...
      if (typeof inProcessOptions.isWebView != 'boolean')
        isWebView = inProcessOptions.isWebView;
    }
    ...
    function initWindow(done)
    {
        webContents = win.webContents || win.getWebContents();
        if (typeof isWebView != 'boolean')
          isWebView = !!webContents.hostWebContents;
        else if (!webContents.hostWebContents) // webview should have a hostWebContents
        {
          parent.emit('console','error','isWebView but not hostWebContents');
        }
        // If controlling a <webview> then ipc messaging is between BrowserWindow and webview and webview preload will
        // emit messages on the webview object instead of ipcMain
        windowIpc = isWebView ? win : require('electron').ipcMain;
        /**
         * Window Docs:
         * https://github.com/atom/electron/blob/master/docs/api/browser-window.md
         */

        frameManager = FrameManager(win,parent);
        ...
        function onConsoleEvent(event,type,args)
        {
          parent.emit.apply(parent, ['console', type].concat(args));
        }

        // TODO: Move on 'destroy'
        var webViewIpcMessage = function(event)
        {
          if (event.channel == buildChannelName('page'))
            onPageEvent.apply(null,event.args);
          else if (event.channel == buildChannelName('console'))
            onConsoleEvent(event,event.args[0],event.args.slice(1));

        };

        if (isWebView)
          windowIpc.addEventListener('ipc-message',webViewIpcMessage)
        else {
          windowIpc.on(buildChannelName('console'), onConsoleEvent);
          windowIpc.on(buildChannelName('page'), onPageEvent)
        }
        ...
        webContents.on('did-start-loading', function() {
            if (webContents.isLoadingMainFrame()) {
              if (!isWebView)
              {
                webContents.preload = function () {
                  return buildInitOptions()
                }
              }
    ...
    parent.respondTo('goto', function(url, headers, initFn, timeout, done) {

        if (!url || typeof url !== 'string') {
            return done('goto: `url` must be a non-empty string');
        }
        currentGotoFn = String(initFn);
        var httpReferrer = '';
        var extraHeaders = '';
        ...
        if (webContents.getURL() == url) {
            ...
            function handleFinish(event) {
                webContents.send('init',buildInitOptions());
    ...
    function buildInitOptions()
    {
      let a = {
        ...
        sendToHost: isWebView,
        initFn: String(currentGotoFn).length ? currentGotoFn : null,
        executeJavaScriptSecure: windowOptions.executeJavaScriptSecure
      }
      return a;
    ...
    parent.respondTo('javascript', function(src, args,_done) {
        var doIpcInit = true;
        var ipcInitIntervalId;
        ...
        {
          if (ipcInitIntervalId)
            clearInterval(ipcInitIntervalId);
          _done(err,response);
        }

        var responseCalled = false;
        var response = function(event,response) {
            if (responseCalled)
              return;
            responseCalled = true;
            if (isWebView)
            {
              windowIpc.removeEventListener('ipc-message', webViewIpcMessage);
            }
            else
            {
              windowIpc.removeListener(buildChannelName('error'), error);
              windowIpc.removeListener(buildChannelName('log'), log);
            }
            done(null, response);
        };

        var errorCalled = false;
        var error = function(event,error) {
            if (errorCalled)
              return;
            errorCalled = true;
            if (isWebView) {
            ...
            {
              windowIpc.removeListener(buildChannelName('log'), log);
              windowIpc.removeListener(buildChannelName('response'), response);
            }
            done(error);
        };
        ...
        var webViewIpcMessage = function(event)
        {
          if (event.channel == buildChannelName('response'))
            response(event,event.args[0]);
          else if (event.channel == buildChannelName('error'))
            error(event,event.args[0]);
          else if (event.channel == buildChannelName('log'))
            log(event,event.args);
          else if (event.channel == buildChannelName('init-received'))
            ipcInitReceived();
        };
        ...
        if (isWebView)
        {
          ...
          windowIpc.once(buildChannelName('response'), response);
          windowIpc.once(buildChannelName('init-received'), ipcInitReceived);
          windowIpc.once(buildChannelName('error'), error);
          windowIpc.on(buildChannelName('log'), log);
        }

        //parent.emit('log', 'about to execute javascript: ' + src);

        if (!!windowOptions.executeJavaScriptSecure)
        {
          ipcInitIntervalId = setInterval(sendIpcInit,1000);
          sendIpcInit();
        }
        else
        {
        ...
        function doExecute()
        {
          if (windowOptions.executeJavaScriptSecure) // execute via IPC
          {
    ...
    parent.respondTo('size', function(width, height, done) {
        var setSize = win.setSize ? win.setSize.bind(win) : webContents.setSize.bind(webContents);
    ...
    parent.respondTo('action', function(name, fntext, done){
        ...
        fn(name, windowOptions, parent, win, windowIpc, function(error){
            done(error);
        });
    });
    ...
    parent.respondTo('continue', function(done) {
        if (isReady()) {
            parent.emit('log', 'continue: window is ready');
            done();
        } else {
            parent.emit('log', 'continue: waiting for window to load...');
            if (isWebView)
              win.addEventListener('did-change-is-ready',didChangeIsReady);
            else
              win.once('did-change-is-ready', didChangeIsReady);

            var didChangeIsReadyCalled = false;
            function didChangeIsReady()
            {
              if (didChangeIsReadyCalled)
                return;
              didChangeIsReadyCalled = true;
              if (isWebView)
                win.removeEventListener('did-change-is-ready',didChangeIsReady);

              parent.emit('log', 'continue: window became ready: ' + webContents.getURL());
              done();
    ...
    function setIsReady(ready) {
        ...
        if (ready !== win[IS_READY]) {
            win[IS_READY] = ready;
            if (isWebView)
              win.dispatchEvent(new Event('did-change-is-ready'))
            else
