...
exports.engineVersions = function(done){
  this.debug(debugActions,'.engineVersions()');
...
exports.title = function(done) {
  this.debug(debugActions,'.title() getting it');
...
exports.url = function(done) {
  this.debug(debugActions,'.url() getting it');
  this.evaluate_now(function() {
...
exports.path = function(done) {
  this.debug(debugActions,'.path() getting it');
  this.evaluate_now(function() {
...
exports.visible = function(selector, done) {
  this.debug(debugActions,'.visible() for ' + selector);
...
exports.exists = function(selector, done) {
  this.debug(debugActions,'.exists() for ' + selector);
  this.evaluate_now(function(selector) {
...
exports.click = function(selector, done) {
  this.debug(debugActions,'.click() on ' + selector);
  this.evaluate_now(function (selector) {
...
exports.mousedown = function(selector, done) {
  this.debug(debugActions,'.mousedown() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.mouseup = function(selector, done) {
  this.debug(debugActions,'.mouseup() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.mouseover = function(selector, done) {
  this.debug(debugActions,'.mouseover() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.mouseout = function(selector, done) {
  this.debug(debugActions,'.mouseout() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.type = function() {
  var selector = arguments[0], text, blur, done;
  if(arguments.length == 2) {
    done = arguments[1];
    blur = this.options.blurInput;
  }
  ...
  this.debug(debugActions,'.type() %s into %s', text, selector);
  var self = this;

  focusSelector.bind(this)(function(err) {
    if(err) {
      self.debug(debugActions,'Unable to .type() into non-existent selector %s', selector);
      return done(err);
    }

    var blurDone = blur ? blurSelector.bind(this, done, selector) : done;
    if ((text || '') == '') {
      this.evaluate_now(function(selector) {
...
exports.insert = function(selector, text, blur,done) {
  if (arguments.length === 2) {
    done = text;
    text = null;
    blur = this.options.blurInput;
  ...
  this.debug(debugActions,'.insert() %s into %s', text, selector);
  var child = this.child;

  focusSelector.bind(this)(function(err) {
    if(err) {
      self.debug(debugActions,'Unable to .insert() into non-existent selector %s', selector);
      return done(err);
    }
...
exports.check = function(selector, done) {
  this.debug(debugActions,'.check() ' + selector);
  this.evaluate_now(function(selector) {
    var element = document.querySelector(selector);
...
exports.uncheck = function(selector, done){
  this.debug(debugActions,'.uncheck() ' + selector);
  this.evaluate_now(function(selector) {
      var element = document.querySelector(selector);
      var event = document.createEvent('HTMLEvents');
...
exports.select = function(selector, option, done) {
  this.debug(debugActions,'.select() ' + selector);
...
exports.forward = function(done) {
  this.debug(debugActions,'.forward()');
  this.evaluate_now(function() {
...
exports.refresh = function(done) {
  this.debug(debugActions,'.refresh()');
  this.evaluate_now(function() {
...
exports.wait = function () {
  ...
  if (args.length < 2) {
    this.debug(debugActions,'Not enough arguments for .wait()');
    return done();
  }

  var arg = args[0];
  if (typeof arg === 'number') {
    this.debug(debugActions,'.wait() for ' + arg + 'ms');
    ...
    this.debug(debugActions,'.wait() for '+arg+' element'+(timeout ? ' or '+timeout+'msec' : ''));
    waitelem.apply({ timeout: timeout }, [this, arg, done]);
  }
  else if (typeof arg === 'function') {
    this.debug(debugActions,'.wait() for fn');
    args.unshift(this);
...
exports.evaluate = function (fn/**, arg1, arg2..., done**/) {
  ...
  this.debug(debugActions,'.evaluate() fn on the page');
  var timeoutTimer = setTimeout(function(){
...
exports.inject = function (type, file, done) {
  this.debug(debugActions,'.inject()-ing a file');
  if (type === 'js') {
    ...
    this.debug(debugActions,'unsupported file type in .inject()');
    done();
  }
};
...
exports.viewport = function (width, height, done) {
  this.debug(debugActions,'.viewport()');
...
exports.useragent = function(useragent, done) {
  this.debug(debugActions,'.useragent() to ' + useragent);
...
exports.scrollTo = function (y, x, done) {
  this.debug(debugActions,'.scrollTo()');
...
exports.screenshot = function (path, clip, done) {
  this.debug(debugActions,'.screenshot()');
  ...
  this.child.call('screenshot', path, clip, function (error, img) {
    ...
    this.debug(debugActions,'.screenshot() captured with length %s', buf.length);
...
exports.html = function (path, saveType, done) {
  ...
  this.child.call('html', path, saveType, function (error) {
    if (error)  this.debug(debugActions,error);
    done(error);
  });
}
...
exports.pdf = function (path, options, done) {
  ...
  this.child.call('pdf', path, options, function (error, pdf) {
    if (error) self.debug(debugActions,error);
    var buf = new Buffer(pdf.data);
    self.debug(debugActions,'.pdf() captured with length %s', buf.length);
    path ? fs.writeFile(path, buf, done) : done(null, buf);
  });
};
...
exports.cookies.get = function (name, done) {
  this.debug(debugActions,'cookies.get()')
  var query = {}
...
exports.cookies.set = function (name, value, done) {
  this.debug(debugActions,'cookies.set()')
  var cookies = []
...
exports.cookies.clear = function (name, done) {
  this.debug(debugActions,'cookies.clear()')
  var cookies = []
 ...
 */
 ...
 exports.authentication = function (login, password, done) {
   this.debug(debugActions,'.authentication()');
